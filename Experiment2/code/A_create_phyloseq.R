...
### Subset and filter ASV table
# Because assignment of ASV's to taxa is a computationally intensive step, will create an ASV table subset to only ASV's that will be in final analysis.  
# Remove ASV's that have overall low abundance
temp_asv <- data.frame(t(asv_table_nochim))
...
final_filtered_asv_table_t <- final_filtered_asv_table_t[rownames(meta),]
ASVs_filt_renamed = DNAStringSet(colnames(final_filtered_asv_table_t))
names(ASVs_filt_renamed) = paste0("ASV", 1:ncol(final_filtered_asv_table_t))
...
meta <- meta[meta$Experiment == "Experiment 2",]
